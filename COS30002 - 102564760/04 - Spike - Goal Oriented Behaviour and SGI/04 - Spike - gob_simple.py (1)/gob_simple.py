...
def action_utility(action, goal):
    ...
    if goal in actions[action]:
        ...
        u = -actions[action][goal]
        if (actions[action][goal] < 0) and ((goals[goal] + actions[action][goal]) < 0):
            u -= actions[action][othergoal] 
        elif (actions[action][goal] < 0) and ((goals[goal] + actions[action][goal]) >= 0):
            u += 2
        return u
    else:
    ...
    for key, value in actions.items():
        ...
        if best_goal in value:
            ...
            # Is this new action better than the current action?
                ...
                if (action_utility(key, best_goal) > best_utility):
