...
class Agent(object):
    ...
    def __init__(self, world=None, scale=30.0, mass=1.0, mode='group'):
    ...
    def calculate(self, delta):
        ...
        self.TagNeighbours(self.world.agents, self.world.radius)        
        force += self.wander(delta)         
        force += self.Separation(self.world.agents) * self.world.seperation
    ...
    def flee(self, hunter_pos):
        ...
        if ((hunter_pos - self.pos).normalise()).length() < 5.0:
    ...
    def wander(self, delta):
        '''  randomwandering using a projected jitter circle '''
        wt = self.wander_target# this behaviour is dependent on the update rate, so this line must# be included when using time independent framerate. 
        jitter_tts = self.wander_jitter * delta # this time slice# first, add a small random vector to the target'sposition 
        wt += Vector2D(uniform(-1,1) * jitter_tts, uniform(-1,1) * jitter_tts)# re-project this new vector back on to a unit circle
        wt.normalise()# increase the length of the vector to the same as the radius# of the wander circle
        wt *= self.wander_radius# move the target into aposition WanderDist in front of the agent
        target = wt + Vector2D(self.wander_dist, 0)# project the target into world space
